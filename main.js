...
d3.csv("nobel_laureates.csv").then((data) => {
  ...
  const categories = d3.rollup(
    ...
    (d) => d.categoryGroup // Group by new property that was just created
  );
  ...
  // 4: SET SCALES
  const allYears = Array.from(categories.values()).flatMap((yearMap) =>
    Array.from(yearMap.keys())
  );
  const yearCounts = Array.from(categories.values()).map((categoryMap) =>
  ...
  svgLine
    ...
    .attr("d", (d) => {
      const yearMap = d[1];
      const values = Array.from(yearMap.entries()).map(([year, count]) => ({
        year,
        count,
      }));
      return line(values);
  ...
  svgLine.append("g").call(d3.axisLeft(yScale).tickFormat(d3.format("d")));
